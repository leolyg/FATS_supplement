...
class ShadowModels:
    ...
    def train_predict_shadows(self, **fit_kwargs):
        ...
        # TRAIN and predict
        results = []
        for model, data_subset in zip(self.models, self._splits):
            X = data_subset[:, :-1]
            y = data_subset[:, -1]
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.5)

            # convert y_train to one-hot encoding in numpy
            # y_tmp = np.eye(self.target_classes)[y_train.astype(int)]
            # print(f'y_tmp shape: {y_tmp.shape}')
            model.fit(X_train, y_train, **fit_kwargs)
            # data IN training set labelet 1
            y_train = y_train.reshape(-1, 1)
            predict_in = model.predict_proba(X_train)
            # project the prediction to the target model classes based on y_train
            if predict_in.shape[1] != self.target_classes:
                predict_space = np.zeros((predict_in.shape[0], self.target_classes))
                existing_classes = np.unique(y_train)
                for i in np.arange(predict_in.shape[0]):
                    predict_space[i, existing_classes] = predict_in[i]
                predict_in = predict_space              
            ...
            # project the prediction to the target model classes based on y_train
            if predict_out.shape[1] != self.target_classes:
                predict_space = np.zeros((predict_out.shape[0], self.target_classes))
                existing_classes = np.unique(y_train)
                for i in np.arange(predict_out.shape[0]):
                    predict_space[i, existing_classes] = predict_out[i]
                predict_out = predict_space    
